...
const VOICES: Record<string, PersonalityVoice> = {
  ...
  "R32": { // Albertito
    ...
    ownGoal: (s, m) => pick([`En contra de ${s} en el ${m}'. Más autogol que mi candidatura`, `Autogol de ${s} en el ${m}'. Peor que mis declaraciones en cadena nacional`, `${s} la metió en su propio arco en el ${m}'. Como cuando yo hablé de los barcos`, `En contra en el ${m}'. ${s} se mandó una macana peor que la fiesta de Olivos`, `¡Autogol de ${s}! Minuto ${m}. Esto es culpa de Macri, seguro`]),
    redCard: (p, m) => pick([`Roja para ${p} en el ${m}'. Se va más rápido que yo de Olivos`, `${p} expulsado. Como yo del poder, pero más dignamente`, `${p} se fue en el ${m}'. Lo echaron mejor que a mí`, `Roja para ${p} en el ${m}'. Se va como Guzmán del ministerio`, `${p} afuera en el ${m}'. Duró más que algunos de mis ministros`]),
    yellowCard: (p, m) => pick([`Amarilla para ${p} en el ${m}'. Le pasa por no escuchar a Cristina`, `${p} amonestado. Todavía no lo echaron, le va mejor que a mí`, `${p} con amarilla en el ${m}'. Yo le hubiera dado un cargo en vez de una tarjeta`, `Amarilla para ${p}. Minuto ${m}. Se la dedico a Macri`, `${p} amonestado en el ${m}'. Guardo conmigo el dolor de esta tarjeta`]),
    penalty: (s, m) => pick([`¡Penal de ${s} en el ${m}'! Más polémico que la fiesta de Olivos`, `¡Penal! ${s} en el ${m}'. Más cuestionable que mi gestión`, `¡Penal en el ${m}'! ${s} lo patea. Esto es más tenso que una conferencia mía`, `¡${s} con penal! Minuto ${m}. Más drama que mi relación con Cristina`, `¡Penal de ${s} en el ${m}'! La culpa de todo esto es de Macri`]),
    scoreless: (m) => pick([`${m} minutos y 0-0... esto está más trabado que la economía que dejé`, `¿No piensan meter un gol? Esto es peor que mi gestión`, `0-0 al minuto ${m}. Más vacío que las arcas del Estado`, `${m}' sin goles. Heredamos un partido sin goles y no lo pudimos resolver`, `Minuto ${m} y 0-0. Esto lo miro por zoom y me duermo igual`]),
    ...
    comodinLosing: (n, h, a) => pick([`${n} puso el comodín con el ${h}-${a}... F. Como mi legado`, `El comodín de ${n} llora como el presupuesto nacional`, `${n} sufre con el comodín y el ${h}-${a}. Bienvenido a mi mundo`, `${n} pierde con el comodín. ${h}-${a}. Más triste que mi despedida de Olivos`, `El comodín de ${n} con el ${h}-${a}... esto es culpa de Macri`]),
    comodinDraw: (n, m) => pick([`${n} necesita un golcito... minuto ${m} y empate. Como yo necesitaba un milagro económico`, `El comodín de ${n} tiembla con este empate. Guardo conmigo el dolor`, `${n} empata en el ${m}' con el comodín. Más angustia que esperar los datos del INDEC`, `Empate y el comodín de ${n} transpira. Minuto ${m}. Esto es como mi gestión: mucho suspenso y poco resultado`, `${n} con el comodín y empate en el ${m}'. Decime algo lindooooo`]),
    ...
    fewRight: (names) => pick([`Solo ${names} le están pegando. Los demás están como mi gabinete: perdidos`, `${names} nada más aciertan. El resto predice como yo gobernaba`, `Únicamente ${names} aciertan. El resto heredó mis genes de predicción`, `${names} son los únicos. A los demás les recomiendo conectarse al zoom`, `Solo ${names} le pegan. Los demás necesitan un zoom con Cristina para mejorar`]),
    idle: () => pick([
    ...
    taunt: (n) => pick([
      ...
      `${n}, te recomiendo conectarte por zoom como hago yo`,
    ...
    lecture: {
      RSA: ["Yo que fui profesor en la UBA les cuento: Sudáfrica tiene 11 idiomas oficiales. Once. Yo con uno solo ya me metía en problemas", "Sudáfrica fue sede del Mundial 2010. El de la vuvuzela. Peor ruido que mis conferencias de prensa", "Mandela estuvo 27 años preso y después fue presidente. Yo fui presidente y después me sentí preso"],
      CAN: ["Como les enseñaba a mis alumnos: Canadá tiene más lagos que todos los demás países juntos. Dato verificable, no como mis estadísticas del INDEC", "Clase de geografía, como en la UBA: Canadá es el segundo país más grande del mundo. El primero en hockey sobre hielo y en pedir perdón", "Los canadienses dicen 'sorry' por todo. Yo también debería, como les dije a mis alumnos"],
      BRA: ["Brasil tiene 5 mundiales. Nosotros 3. Pero como diría Cristina, no es una competencia... o sí", "Les doy un dato que usaba en la facultad: Brasil es el único país que jugó todos los mundiales. Presencia perfecta. La mía en el Congreso era otra cosa", "Brasil tiene 200 millones de habitantes. Todos opinan de fútbol. Como los 45 millones nuestros"],
      JPN: ["Japón tiene trenes que llegan a horario. Imagínense si acá tuviéramos eso. Se los dije mil veces a mis alumnos en la UBA", "En Japón los hinchas limpian el estadio después del partido. Si hicieran eso acá, el estadio estaría más limpio que mi expediente", "Dato de mi cátedra de derecho: en Japón hay más de 5 millones de máquinas expendedoras. Una cada 23 personas. Más accesibles que un ministro mío"],
      GER: ["Alemania tiene 4 mundiales y el partido más largo de mi vida fue contra ellos en 2014. No quiero hablar de eso", "Yo que soy profesor les enseño: la Bundesliga tiene la mayor asistencia promedio del mundo. Más público que mis actos de campaña", "Los alemanes son famosos por ser puntuales. Yo era puntual para las clases en la UBA, no para las reuniones de gabinete"],
      PAR: ["Como explicaba en la facultad: Paraguay es el único país de Sudamérica con una bandera diferente en cada lado. Como yo, que tenía una cara para cada lado político", "Los guaraníes son uno de los pueblos originarios más orgullosos. Paraguay tiene dos idiomas oficiales: español y guaraní. Más bilingüe que mi doble discurso"],
      NED: ["Holanda está un tercio bajo el nivel del mar. Aun así no se hunden. Nosotros con el dólar... mejor no hablo", "Los holandeses inventaron el microscopio. Si me hubieran mirado a mí con microscopio... mejor cambiemos de tema", "Dato que daba en la UBA: Holanda es el mayor exportador de flores del mundo. Yo también repartía flores... a Cristina, para que no se enoje"],
      MAR: ["Marruecos fue semifinalista en Qatar 2022. La mejor actuación africana en un mundial. Esto lo enseño en mis clases", "El cuscús marroquí es patrimonio de la humanidad. Las empanadas argentinas todavía no, pero deberían", "Marruecos tiene el desierto del Sahara. Más seco que mi relación con Cristina al final del mandato"],
      ...
      SWE: ["Suecia inventó IKEA. Yo también armé un gabinete con instrucciones confusas y sobraron piezas", "Los suecos tienen 'fika', una pausa obligatoria para el café. Yo tenía zoom, que es parecido pero más aburrido", "Dato que mis alumnos de la UBA adoraban: Zlatan Ibrahimovic es sueco. Un tipo que habla de sí mismo en tercera persona. Yo nunca haría eso... o sí"],
      MEX: ["México tiene 35 sitios patrimonio de la humanidad. Más que cualquier país de América. La cancha del Azteca es casi uno más", "El Estadio Azteca recibió 2 finales mundialistas. Ahí Maradona hizo el gol del siglo. Como profesor de la UBA debo decir: fue irregular... pero hermoso", "Dato que enseño en clase: los mexicanos inventaron el chocolate caliente. Sin los aztecas no hay submarino"],
      ...
      COD: ["El Congo tiene el río Congo, el más profundo del mundo. 220 metros. Más profunda que la crisis económica que heredamos... bueno, casi", "Dato de mi cátedra: la República Democrática del Congo es el país más grande de África subsahariana. Grande como mi sueño de ser recordado como buen presidente"],
      USA: ["Como profesor de derecho constitucional les digo: Estados Unidos es el país con más mundiales organizados después de este", "150 millones de estadounidenses van a ver partidos del mundial en su país. Más que la población de Argentina, Paraguay y Uruguay juntos"],
      ...
      CRO: ["Croacia tiene 4 millones de habitantes y fue finalista en 2018 y semifinalista en 2022. Per cápita, la mejor selección del mundo", "Les enseño algo: la corbata es invento croata. Cravat viene de 'croata'. Yo prefiero no usar corbata, por las dudas"],
      ESP: ["España ganó el mundial 2010 con el tiki-taka. 800 pases por partido. Más vueltas que yo para responder una pregunta", "En mi cátedra siempre tiraba este dato: España tiene más bares per cápita que cualquier país europeo. El verdadero dato importante"],
      ...
      EGY: ["Yo que enseñé derecho en la UBA les digo: Egipto tiene las pirámides que tienen 4.500 años. Más estables que la economía argentina. Mucho más", "Mohamed Salah es el máximo goleador histórico de Egipto. Un fenómeno. Si cobrara en pesos sería triste"],
      SUI: ["Suiza tiene más bancos que canchas de fútbol. Y más secretos bancarios que yo secretos de Estado... bueno, casi", "Los suizos son neutrales desde 1815. Yo intenté ser neutral entre Cristina y Macri y no me salió", "Como les decía a mis alumnos: Suiza tiene 4 idiomas oficiales. Y el chocolate más caro del mundo. Prioricen el chocolate"],
      ...
      GHA: ["Dato de mi cátedra: Ghana fue el primer país de África subsahariana en independizarse, en 1957. Importante para el derecho internacional", "Los ghaneses le dicen 'Black Stars' a su selección. Tienen 4 Copas Africanas. Historial respetable"],
    },
  },
};
