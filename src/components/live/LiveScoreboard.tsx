...
const VOICES: Record<string, PersonalityVoice> = {
  ...
  "R32": { // Albertito
    goal: (s, m, _side, h, a) => pick([
      ...
      `¡Gol de ${s}! ${h}-${a}. Como panelista invitado digo: lo vi venir. Como presidente no vi venir nada, pero ahora sí`,
      `${s} en el ${m}'. ${h}-${a}. Golazo. Va a mis memorias, capítulo doce: cosas que salieron bien, de otros`,
      `¡${s} convirtió! ${h}-${a}. Yo hubiera metido el gol de otra manera`,
      `${h}-${a}. ${s}, minuto ${m}. Esto merece cadena nacional. Yo ya no tengo, pero merece`,
      `¡Gol! ${s} en el ${m}'. Dylan ladró en Olivos. Dylan siempre sabe antes que todos`,
    ]),
    ownGoal: (s, m) => pick([`Autogol de ${s} en el ${m}'. Conozco el género: yo era arquero y presidente. Me metía goles en contra en los dos puestos`, `En contra de ${s}. Un clásico: trabajás para adelante y la pelota entra para atrás. Mi mandato en una sola jugada`, `En contra en el ${m}'. ${s} se mandó una macana peor que la fiesta de Olivos`, `¡Autogol de ${s}! Minuto ${m}. Esto es culpa de Macri, seguro`]),
    redCard: (p, m) => pick([`Roja para ${p} en el ${m}'. Se va como Guzmán del ministerio`, `${p} expulsado. Cristina decía que había funcionarios que no funcionaban. Árbitros que funcionan: mirá vos, existen`, `${p} afuera en el ${m}'. Al menos a él le mostraron una tarjeta. A mí me echaron por Twitter`, `Roja para ${p}. Se va en silencio, sin conferencia de prensa. Un profesional`, `${p} afuera en el ${m}'. Duró más que algunos de mis ministros`]),
    yellowCard: (p, m) => pick([`Amarilla para ${p} en el ${m}'. Le pasa por no escuchar a Cristina`, `${p} amonestado. Un apercibimiento simbólico, como los que yo le hacía a la inflación`, `${p} con amarilla en el ${m}'. Que se cuide: la reincidencia se paga. Lo digo por experiencia electoral`, `Amarilla para ${p}. Yo le hubiera dado un cargo en vez de una tarjeta. Por eso me fue como me fue`]),
    penalty: (s, m) => pick([`¡Penal de ${s} en el ${m}'! Más polémico que la fiesta de Olivos`, `¡Penal en el ${m}'! ${s} agarra la pelota. Yo también agarré algo grande una vez: un país. No salió bien, pero el gesto estaba`, `¡${s} con penal! Minuto ${m}. Más drama que mi relación con Cristina`, `¡Penal! ${s} desde los doce pasos. Doce pasos también me separaban del despacho de Cristina y jamás los caminé`]),
    scoreless: (m) => pick([`Es una mierda de partido, seguro lo produjo Sandra.`, `${m} minutos sin goles. Esto necesita medidas urgentes. Un DNU, algo`, `0-0 al minuto ${m}. Partido conservador. Ni mi gabinete era tan conservador, y eso que no hacía nada`, `${m}' sin goles. Heredamos un partido sin goles y no lo pudimos resolver`, `Minuto ${m} y 0-0. Pepe Mujica debe estar torrando.`]),
    lateGame: (h, a) => pick([`Últimos minutos, ${h}-${a}. Los finales se definen ahora. Los finales son lo mío: pregúntenle a mi mandato`, `Se termina esto. ${h}-${a}. Como mi mandato, con más pena que gloria`, `${h}-${a} y queda nada. Yo en los finales me iba a Olivos. Ahora me quedo hasta el pitazo: soy un profesional del comentario`, `${h}-${a} en el cierre. Más suspenso que la noche de las elecciones del 2023`]),
    ...
    idle: () => pick([
      ...
      "El profe me invitó a debatir. Cristina jamás me invitó ni a las reuniones",
      "Cristina decía que había funcionarios que no funcionaban. Ahora funciono: opino de fútbol",
      "Estoy escribiendo mis memorias. Este partido va al capítulo de cosas que no fueron culpa mía. O sea, a cualquiera de los capítulos",
      "Me dejan opinar de fútbol. De economía ya no me dejan opinar ni en el asado familiar",
      "El VAR es como la lapicera: todos saben quién la tiene, nadie sabe cuándo la va a usar",
      "Dylan mira el partido conmigo en Olivos... bueno, en mi departamento. Dylan nunca me juzgó. Por eso lo quiero más que a la militancia",
    ...
    taunt: (n) => pick([
      ...
      `${n}, tu predicción envejeció peor que mis conferencias de prensa`,
